...
def auth_flow(sock):
    """
    Returns (username,nickname) on success, or None on failure.
    Protocol:
      - client sends {"type":"signup"/"login","username":..,"password":..}
      - server replies {"status":"ok"} or {"status":"error", "msg":..}
      - on ok, client must send registration JSON {"nick":..., "pub":[n,e]}
    """
    while True:
        print("Authentication:")
        print("1 - Login")
        print("2 - Create account (signup)")
        ...
        req = {"type": typ, "username": username, "password": password}
        ...
        if resp.get("status") == "ok":
            print(resp.get("msg","Success"))
            return username
        else:
            print("Error:", resp.get("msg", "Unknown error"))
            # loop to try again
...
def main():
    ...
    username = auth_flow(s)
    if username is None:
        s.close()
        return

    # 2) After successful auth: choose nickname and cipher
    print("connected")
    nickname = input("Choose your chat nickname (display name): ").strip() or username
    ...
    if choice == '2':
        method = "vigenere"
        key = input("Enter Vigenere key (word): ").strip()
    elif choice == '3':
        method = "substitution"
        key = input("Enter substitution key (26 letters): ").strip()
    else:
        method = "caesar"
        while True:
            try:
    ...
    # 3) Generate RSA keypair and register with server (public key sent)
    ...
    registration = {"nick": nickname, "pub":[str(n), str(e)]}
    # send registration JSON after auth success
